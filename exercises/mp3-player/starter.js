window.addEventListener('load', function () {
  ...
  const palyerDuration = document.querySelector('.player-duration');
  const remaining = document.querySelector('.player-remaining');
  const progressBar = document.querySelector('#progress-bar');
  const playerImage = document.querySelector('.player-image');
  let playing = true;
  const list = ['holo.mp3', 'home.mp3', 'spark.mp3', 'summer.mp3', 'kbtl.mp3'];
  let songIndex = 0;
  ...
  function displayTimer() {
    ...
    if (!duration) {
      palyerDuration.textContent = '0:00';
    } else {
      palyerDuration.textContent = formatTimer(duration);
    }
  }
